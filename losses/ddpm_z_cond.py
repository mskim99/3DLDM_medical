...
class DDPM(nn.Module):
    ...
    def model_predictions(self, x, cond, t, x_self_cond=None, clip_x_start=False):
        if cond == None:
            ...
            model_output = (1 + self.w) * self.model(x, cond, t, x_self_cond) - self.w * self.model(x, cond * 0, t,
                                                                                                    x_self_cond)
        ...
        if self.parameterization == 'eps':
            ...
            x_start = self.predict_start_from_noise(x[:, 0:int(x.shape[1] / 2), :], t, pred_noise)
    ...
    def ddim_sample(self, shape, cond, clip_denoised=True):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            img = x_start * alpha_next.sqrt() + \
                  c * pred_noise + \
                  sigma * noise[:, 0:int(noise.shape[1] / 2), :]

            img = torch.concatenate([img, img_prev[:, int(img_prev.shape[1]/2):img_prev.shape[1], :]], axis=1)
    ...
    def sample(self, batch_size=8, cond=None, return_intermediates=False, idx_cond=None):
        ...
        if self.is_ddim_sampling:
            return self.ddim_sample((batch_size, channels * 2, image_size), idx_cond)
        else:
            return self.p_sample_loop((batch_size, channels * 2, image_size), cond,
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        noise_half = noise[:, 0:int(noise.shape[1] / 2), :]
