...
class DDPM(nn.Module):
    ...
    def __init__(self,
        ...
        # self.image_size = 512 # res 128
        self.image_size = 64  # res 64
    ...
    def model_predictions(self, x, cond, t, x_self_cond=None, mask=None, clip_x_start=False):
        if cond == None:
            model_output = self.model(x, cond, t, x_self_cond)
        else:
            model_output = (1 + self.w) * self.model(x, cond, t, x_self_cond, mask) - self.w * self.model(x, cond * 0, t,
                                                                                                    x_self_cond, mask)
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, cond, mask, clip_denoised=True):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(img, cond, time_cond, self_cond, mask=mask,
    ...
    def ddim_sample_no_tqdm(self, shape, cond, mask, clip_denoised=True):
        batch, device, total_timesteps, sampling_timesteps, eta = shape[
                                                                      0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta
    ...
    @torch.no_grad()
    def sample(self, batch_size=8, cond=None, return_intermediates=False, idx_cond=None, mask=None):
    ...
    def sample_3d(self, batch_size=3, channels=1, cond=None, return_intermediates=False, idx_cond=None, mask=None, tqdm=True):
        image_size = self.image_size
        # channels = self.channels
        ...
        if tqdm:
            ...
            return self.ddim_sample_no_tqdm((batch_size, channels, image_size), idx_cond, mask)
    ...
    def p_losses(self, x_start, cond, t, c_x, c_m, noise=None):
    ...
    def forward(self, x, cond=None, c_x=None, c_m=None, *args, **kwargs):
        # b, c, h, w, device, img_size, = *x.shape, x.device, self.image_size
        # assert h == img_size and w == img_size, f'height and width of image must be {img_size}'
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=x.device).long()
        return self.p_losses(x, cond, t, c_x, c_m, *args, **kwargs), t
