...
class TimeSformerEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.final_fc = nn.Linear(256, dim)
    ...
    def forward(self, video, cond, lc, frame_mask = None):
        ...
        # x = rearrange(video, 'b f c (h p1) (w p2) -> b (f h w) (p1 p2 c)', p1=p, p2=p) # for h, w (xy)
        # x = rearrange(video, 'b (f p3) c (h p1) (w p2) -> b (f h w) (p1 p2 p3 c)', p1=p, p2=p, p3=p) # xyz patch extraction
        # x_2d = rearrange(video, 'b (f p1) c h (w p2) -> b (f h w) (p1 p2 c)', p1=p, p2=p)  # for f, w (xz)
        # x_2d = rearrange(video, 'b (f p1) c (h p2) w -> b (f h w) (p1 p2 c)', p1=p, p2=p) # for f, h (yz)
        # print(x.shape)
    ...
    def __init__(
        ...
        *,
        ...
        for _ in range(depth):
            ff = FeedForward(128, dropout = ff_dropout, label_conc = 128)
            time_attn = Attention(128, dim_head = dim_head, heads = heads, dropout = attn_dropout, label_conc = 128)
            spatial_attn = Attention(128, dim_head = dim_head, heads = heads, dropout = attn_dropout, label_conc = 128)

            time_attn, spatial_attn, ff = map(lambda t: PreNorm(256, t), (time_attn, spatial_attn, ff))
