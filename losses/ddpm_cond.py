...
class DDPM(nn.Module):
    ...
    def model_predictions(self, x_concat, x, cond, t, x_self_cond=None, clip_x_start=False):
        if cond == None:
            model_output = self.model(x_concat, cond, t, x_self_cond)
        else:
            model_output = (1 + self.w) * self.model(x_concat, cond, t, x_self_cond) - self.w * self.model(x_concat, cond * 0, t,
                                                                                                    x_self_cond)
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, cond, clip_denoised=True, init_context=None):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            # print(cond)
            '''
            pred_noise, x_start, *_ = self.model_predictions(img, cond, time_cond, self_cond,
                                                             clip_x_start=clip_denoised)
                                                             '''
            pred_noise, x_start, *_ = self.model_predictions(img_concat, img, cond, time_cond, self_cond,
                                                             clip_x_start=clip_denoised)
    ...
    @torch.no_grad()
    def sample(self, batch_size=4, cond=None, return_intermediates=False, idx_cond=None, init_context=None):
        image_size = self.image_size
        channels = self.channels

        ''' ddim sampling '''
        if self.is_ddim_sampling:
            return self.ddim_sample((batch_size, channels, image_size), idx_cond, init_context=init_context)
        else:
    ...
    def p_losses(self, x_start, cond, t, noise=None, init_context=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_init = torch.zeros(x_noisy.shape).cuda()
        if init_context is not None:
            x_init = init_context
        # print(x_noisy.shape)
        # print(x_init.shape)
        x_input = torch.cat((x_noisy, x_init), axis=1)
        # print(x_input.shape)
        # model_out = self.model(x_noisy, cond, t)
        model_out = self.model(x_input, cond, t)
        ...
        return loss, loss_dict, x_init

    def forward(self, x, cond=None, init_context=None, *args, **kwargs):
        # b, c, h, w, device, img_size, = *x.shape, x.device, self.image_size
        # assert h == img_size and w == img_size, f'height and width of image must be {img_size}'
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=x.device).long()
        return self.p_losses(x, cond, t, init_context=init_context, *args, **kwargs), t
